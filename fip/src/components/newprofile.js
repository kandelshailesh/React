...
axios({
    ...
    .then(function (response) {
        ...
        socket.emit('status',{'status':status,'email':email,'fname':fname,'lname':lname,'url':urldata,'filename':response.data.filename})
...
if(e.target.style.color==='black'){
  ...
  socket.emit('like',{'imageid':e.target.getAttribute('name')})
}
else{
  e.target.style.color='black';
  console.log(e.target.getAttribute('name'))
  socket.emit('unlike',{'imageid':e.target.getAttribute('name')})
}
}
...
<a>{result.uploaderemail}</a>
<br/>
<br/>
<img style={{width:'100%'}} src={images(`./${result.file_name}`)} />
<div  className="container">
<div className="row">
{result.like>0 ?<button ref={(ref) => { this.likeButton = ref; }} onClick={(e)=>this.likebutton(e)} name={result._id}style={{border:'0px',color:'blue',backgroundColor:'#EEE'}} className="col-sm-4"><i  className="fa fa-thumbs-o-up"></i><br/>{result.like}</button>:<button ref={(ref) => { this.likeButton = ref; }} onClick={(e)=>this.likebutton(e)} name={result._id}style={{border:'0px',color:'black',backgroundColor:'#EEE'}} className="col-sm-4"><i  className="fa fa-thumbs-o-up"></i><br/>{result.like}</button>
}
  <button onClick={(e)=>this.commentbutton(e)} style={{border:'0px',backgroundColor:'#EEE'}}  className="col-sm-4"><i  style={{color:'black'}} className="fa fa-comment"></i> <br/>{result.comment}</button>
